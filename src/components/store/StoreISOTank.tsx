...
import {
  FiArrowRight,
  FiGrid, FiLayers, FiPackage, FiShield,
} from 'react-icons/fi';
...
import heroImgRaw from '@data/images/store/iso/ISO-Tank-hero.png';
      ...
      <section className="bg-secondary py-20 px-6 md:px-12">
        <div className="container mx-auto max-w-[var(--max-width)]">
          <motion.div initial="hidden" whileInView="visible" viewport={viewportOnce} variants={containerVariants}>
            <SectionHeader dark eyebrow="Certified & compliant" head={tc(sLabel('S1 Specifications & certifications'))}
              desc="International-standard tank containers for safe handling of liquid chemicals and food-grade products." />
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-px bg-primary/5 mt-2">
              {s1Specs.map(([title, desc], i) => (
            ...
            <div className="border border-secondary/10 rounded-sm p-6 sm:p-10 md:p-16 flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8">
              <div className="max-w-xl">
                <motion.span variants={itemVariants}
                  ...
                  Certified liquid cargo storage
                </motion.span>
                <motion.h2 variants={itemVariants}
                ...
                <motion.p variants={itemVariants}
                  ...
                  Available for lease, pan-India. Tell us your requirement and we'll respond within 24 hours.
                </motion.p>
              </div>
