...
const MovePage: React.FC = () => {
  const liveData = useCmsData('move', moveData)
  const { hero, services, why_crystal } = liveData.page.sections.reduce((acc: any, section: any) => {
    acc[section.type] = section;
    return acc;
  }, {});

  const serviceIcons = [<FiTruck />, <FiMapPin />, <FiPackage />];
      ...
      <section className="relative h-[100vh] min-h-[600px] bg-secondary text-primary flex flex-col justify-center overflow-hidden px-6 md:px-12 pt-20">
        ...
        >
          ...
          <h1 className="font-heading font-extrabold text-h1 leading-tight-none tracking-tighter mb-8 text-primary max-w-5xl">
            <CharReveal text={tc(hero.headline.split(' ').slice(0, 4).join(' '))} />
            <CharReveal text={tc(hero.headline.split(' ').slice(4).join(' '))} className="text-accent" />
          </h1>
        ...
        <div className="container mx-auto max-w-5xl text-center flex flex-col items-center">
          <motion.div initial="hidden" whileInView="visible" viewport={viewportOnce} variants={containerVariants}>
            <LocalSectionHeader
              dark
              align="center"
              head={tc(why_crystal.type.replace(/_/g, ' '))}
            />
            <motion.p variants={itemVariants} className="font-body text-body-lg text-primary/70 leading-relaxed mb-12 max-w-3xl"
